"""Mistral response/usage parsing.

Handles ChatCompletion responses from the ``mistralai`` Python SDK.
Mistral's response format mirrors OpenAI's — ``.model``, ``.usage``,
...
def parse_usage(response: Any) -> UsageRecord:
    """Extract token counts from a Mistral ChatCompletion response.

    Expects *response* to have ``.model`` and ``.usage`` attributes
    (standard ``mistralai`` response).
    ...
    if usage is None:
        raise ValueError("Response is missing 'usage' — ensure the Mistral API returns usage data")
